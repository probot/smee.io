...
let mainMinCssGz, mainMinJsGz
if (isProd) {
  [mainMinJsGz, mainMinCssGz] = ['main.min.js.gz', 'main.min.css.gz'].map(file => fs.readFileSync(path.join(__dirname, '..', 'public', file)))
}
...
const [
  ...
  fastify.get('/public/main.min.css', {
    ...
    if (req.headers['accept-encoding'] && req.headers['accept-encoding'].includes('gzip') && isProd) {
      reply
